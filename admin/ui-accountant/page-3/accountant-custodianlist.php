...
<div class="mb-6">
    ...
    <p class="text-sm text-slate-500 mt-1">Acknowledge noted Purchase Orders from Accounting Staff</p>
</div>
        ...
        <table class="w-full text-xs">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-200">
                    ...
                    <th class="text-left px-3 py-2 font-medium text-slate-600">Prepared By</th>
                ...
                <?php else: ?>
                    <?php foreach ($allPOs as $i => $po): ?>
                        <?php 
                            ...
                            $isAcknowledged = !empty($po['acknowledged_by']);
                        ...
                        <tr class="hover:bg-slate-50 transition-colors <?= $isReplacement ? 'bg-rose-50/40' : '' ?>">
                            ...
                            <td class="px-3 py-2 text-slate-600">
                                <?= $isPending 
                                    ...
                                    : htmlspecialchars($po['noted_by']) ?>
                            </td>
                            <td class="px-3 py-2 text-slate-600">
                                <?= $isAcknowledged 
                                    ? htmlspecialchars($po['acknowledged_by']) 
                                    : '<span class="text-slate-400 italic">—</span>' ?>
                            </td>
                            ...
                            <td class="px-3 py-2 flex items-center gap-1.5">
                                <a href="<?= BASE_URL ?>/accounting-custodianpoview?po_id=<?= $po['id'] ?>"
                                ...
                                <?php if ($isNoted): ?>
                                    <button onclick="acknowledgePO(<?= $po['id'] ?>)"
                                        class="inline-flex items-center gap-1 px-2 py-1 <?= $isReplacement ? 'bg-rose-600 hover:bg-rose-700' : 'bg-indigo-600 hover:bg-indigo-700' ?> text-white text-[11px] font-medium rounded-lg transition-colors">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M5 13l4 4L19 7" />
                                        </svg>
