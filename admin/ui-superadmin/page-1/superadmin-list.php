...
<div class="mb-6">
    ...
    <p class="text-sm text-slate-500 mt-1">Review and approve acknowledged Purchase Orders</p>
</div>
                    ...
                    <?php foreach ($allPOs as $i => $po):
                        ...
                        <tr class="hover:bg-slate-50 transition-colors <?= $isReplacement ? 'bg-rose-50/40' : '' ?>">
                            ...
                            <td class="px-3 py-2">
                                ...
                                <?php else: ?>
                                    <span
                                        class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium bg-green-100 text-green-700">Approved</span>
                                <?php endif; ?>
                            </td>
                            <td class="px-3 py-2">
                                <div class="flex items-center gap-1.5">
                                    <a href="<?= BASE_URL ?>/superadmin-viewpo?po_id=<?= $po['id'] ?>"
                                    ...
                                    <?php if ($isAcknowledged): ?>
                                        <button onclick="approvePO(<?= $po['id'] ?>)"
                                            class="inline-flex items-center gap-1 px-2 py-1 <?= $isReplacement ? 'bg-rose-600 hover:bg-rose-700' : 'bg-green-600 hover:bg-green-700' ?> text-white text-[11px] font-medium rounded-lg transition-colors">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M5 13l4 4L19 7" />
                                            </svg>
...
<script>
    function approvePO(poId) {
        ...
        fetch('<?= BASE_URL ?>/superadmin-backendpoapprove', {
            ...
            .then(r => r.json())
            .then(res => {
                if (res.success) { alert('PO approved successfully!'); window.location.reload(); }
