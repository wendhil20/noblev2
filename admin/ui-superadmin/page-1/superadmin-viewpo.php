<?php
// superadmin-poview.php
include ROOT_PATH . '/network/connect.php';
include ROOT_PATH . '/admin/authentication/index-authguard.php';
include ROOT_PATH . '/admin/authentication/index-roles.php';

// ✅ SUPERADMIN lang ang may access dito
...
<body class="bg-slate-100">
    <div class="ml-60 min-h-screen bg-slate-100 p-6">
        ...
        <div class="flex items-center justify-between mb-6 no-print">
            ...
            <div class="flex gap-2">
                ...
                <!-- ✅ SUPERADMIN STATUS BUTTONS -->
                <?php if ($isPending): ?>
                    <span
                        class="inline-flex items-center gap-2 px-2 py-1.5 bg-yellow-100 text-yellow-700 text-sm font-medium rounded-lg">
                        <i class="fa-solid fa-clock"></i>
                        Awaiting Note
                    </span>
                <?php elseif ($isNoted): ?>
                    <span
                        class="inline-flex items-center gap-2 px-2 py-1.5 bg-blue-100 text-blue-700 text-sm font-medium rounded-lg">
                        <i class="fa-solid fa-clock"></i>
                        Awaiting Acknowledgment
                    </span>
                <?php elseif ($isAcknowledged): ?>
                    <!-- ✅ Superadmin ang mag-aapprove -->
                    <button onclick="approvePO()"
                        class="inline-flex items-center gap-2 px-2 py-1.5 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm">
        ...
        <!-- PO Document — same structure, same fields -->
                    ...
                    <table class="w-full">
                        <tbody>
                            <tr class="border-b border-slate-100">
                                ...
                                <td class="px-4 py-2 text-right font-medium text-slate-800">PHP
                                    <?= number_format($po['subtotal'], 2) ?></td>
                            </tr>
                            <tr class="border-b border-slate-100">
                                <td class="px-4 py-2 text-slate-500">12% VAT Inclusive:</td>
                                <td class="px-4 py-2 text-right text-slate-700">PHP
                                    <?= number_format($po['subtotal'] * 0.12, 2) ?></td>
                            </tr>
                            <tr class="border-b border-slate-100">
                            ...
                            <tr class="border-b border-slate-100">
                                ...
                                <td class="px-4 py-2 text-right text-slate-700">PHP
                                    <?= number_format($po['addl_discount'], 2) ?></td>
                            </tr>
                            <tr class="border-b border-slate-200 bg-amber-50">
                                <td class="px-4 py-2 text-slate-700 font-semibold">Final Amount:</td>
                                <td class="px-4 py-2 text-right font-bold text-slate-900">PHP
                                    <?= number_format($po['final_amount'], 2) ?></td>
                            </tr>
                            <tr class="border-b border-slate-100">
                                <td class="px-4 py-2 text-slate-500">Down Payment:</td>
                                <td class="px-4 py-2 text-right text-slate-700">PHP
                                    <?= number_format($po['down_payment'], 2) ?></td>
                            </tr>
                            <tr class="border-b border-slate-100">
                            ...
                            <tr class="bg-amber-50">
                                ...
                                <td class="px-4 py-2 text-right font-bold text-slate-900">PHP
                                    <?= number_format($po['current_balance'], 2) ?></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="grid grid-cols-5 border-b border-slate-100">
                    ...
                    <div class="px-3 py-3 border-r border-slate-200 flex items-center justify-center min-h-[60px]">
                        <?php if (!empty($po['approved_by_signature'])): ?>
                            <img src="<?= BASE_URL . htmlspecialchars($po['approved_by_signature']) ?>"
                                class="max-h-14 max-w-full object-contain">
                        <?php endif; ?>
                    </div>
                    <div class="px-3 py-3 border-r border-slate-200 flex items-center justify-center min-h-[60px]">
                    ...
                    <div class="px-3 py-3 flex items-center justify-center min-h-[60px]"></div>
                </div>
    ...
    <script>
        // ✅ TAMANG function — approvePO ang superadmin, walang parameter needed (PHP na nag-inject ng poId)
        function approvePO() {
            if (!confirm('Confirm approving this Purchase Order?')) return;

            fetch('<?= BASE_URL ?>/superadmin-backendpoapprove', {
                method: 'POST',
